/** Server-side constants: paths, rate limits, message/frame size caps. */
 ...
 * dist-server/src/server/core/constants.js, so four levels up is the repo
 * root (where dist/, uploads/, and index.html live). Centralized here so
 * other modules don't each guess a `__dirname`-relative depth.
 */
